...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
      ...
      //Находим (кажется, приблизительно) центр фигуры
      ...
      CenterPointX = CenterPointX/(nodesCoords.size() / 3);
      CenterPointY = CenterPointY/(nodesCoords.size() / 3);
      CenterPointZ = CenterPointZ/(nodesCoords.size() / 3);

        // Пройдём по узлам в модели gmsh
        nodes.resize(nodesCoords.size() / 3);
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            // Координаты заберём из gmsh
            double pointX = nodesCoords[i*3]- CenterPointX;
            double pointY = nodesCoords[i*3 + 1]- CenterPointY;
            double pointZ = nodesCoords[i*3 + 2]- CenterPointZ;
            ...
            double vx = pointX; //pow(pointX - pointY, 2);
            double vy = pointY ;
            double vz = pointZ;
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "interstellar/tetr3d-step-" + std::to_string(snap_number) + ".vtu";
